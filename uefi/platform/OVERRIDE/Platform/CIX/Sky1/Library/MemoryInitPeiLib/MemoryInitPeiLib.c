  ...
  )
{
  ...
  for (UINTN Index = 0; MemoryTable[Index].Length != 0; Index++) {
    DEBUG ((DEBUG_INFO, "Memory Map Entry[%d]", Index));
    DEBUG ((DEBUG_INFO, " - PhysicalBase: 0x%016lX", MemoryTable[Index].PhysicalBase));
    DEBUG ((DEBUG_INFO, " - VirtualBase: 0x%016lX", MemoryTable[Index].VirtualBase));
